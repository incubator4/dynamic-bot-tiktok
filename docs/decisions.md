...
| 结果 | 能从 URL 直接解析的作品 / 用户返回本地 `LinkResolution.Preview`（类型、规范化地址、占位标题），不请求平台详情；失败原因用中文 |
| 短链 | 仍匹配 `v.douyin.com`，但不再跟随跳转或使用 Cookie 展开；`resolveLink` 返回中文失败原因，提示改用完整作品或用户主页链接 |
| 请求 | 链接解析不走登录 Cookie、不打作品 / 用户接口，也不计入轮询失败暂停 |
