...
| 结果 | 直链先给本地 `LinkResolution.Preview`（类型、规范化地址、占位标题）。作品再公开读取分享页 / 页面 meta，补作者和封面；失败时仍返回本地预览。失败原因用中文 |
| 短链 | 仍匹配 `v.douyin.com`；不带 Cookie 跟随跳转后再按直链解析。展开失败时 `resolveLink` 返回中文原因，提示改用完整作品或用户主页链接 |
| 请求 | 链接解析不走登录 Cookie，不打需登录的作品 / 用户接口，也不计入轮询失败暂停 |
